          ...
          <div style={{ maxHeight: 130, overflowY: 'auto', border: '1px solid var(--line)', borderRadius: 8, marginBottom: 18 }}>
            {entries.map(e => (
              <div key={e.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 12px', fontSize: 12.5, borderBottom: '1px solid var(--line)' }}>
                <span style={{ color: 'var(--slate)' }}><bdi>{e.entry_type === 'pdf'
                  ? (e.service_items?.length > 0
                      ? (e.service_items.length === 1 ? e.service_items[0].file_name : `${e.service_items.length} files: ${e.service_items.map(i => i.file_name).filter(Boolean).join(', ')}`)
                      : e.file_name)
                  : (e.service_items?.[0]?.description || e.website_renewal_desc || 'Maintenance')}</bdi></span>
